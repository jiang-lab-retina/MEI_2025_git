...
class RGCNet(nn.Module):
    """
    A 3-layer CNN for predicting the max RGC firing rate from visual stimulation,
    ...
    def __init__(self, dropout_rate=0.3):
        ...
        self.fc = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(dropout_rate),
            nn.Linear(128, OUT_DIM)  # output is a scalar (max firing rate)
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        ###########
        #select Y from final_Y_dict that is listed in selected_Y_keys
    ...
    for unit_idx in range(Y_all.shape[0]):
        ...
        # Save model for this unit
        model_filename = f"rgc_model_unit_{unit_idx}.pt"
        torch.save(model.state_dict(), model_filename)
        print(f"Model saved to {model_filename}")
    
        # Collect predictions and true values for scatter plot
        Y_true_all.append(Y_test_tensor.cpu().numpy())
        Y_pred_all.append(Y_pred.cpu().numpy())
    
    # Create a scatter plot across all cells using 5 test samples per unit
