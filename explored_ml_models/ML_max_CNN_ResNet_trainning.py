...
# Define a basic Residual Block for 2D convolutions
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        # If dimensions change, use a convolution in the shortcut path
...
class RGCNet(nn.Module):
    """
    A CNN with residual connections (ResNet blocks) for predicting the max RGC firing rate 
    ...
    def __init__(self, dropout_rate=0.3):
        ...
        # Residual block 1: from 32 to 64 channels with stride 2
        self.resblock1 = ResidualBlock(32, 64, stride=2)  # 100x75 -> 50x38
        # Residual block 2: from 64 to 128 channels with stride 2
        self.resblock2 = ResidualBlock(64, 128, stride=2)  # 50x38 -> 25x19 (floor to 25x19 or 25x18)
        
        # Global Average Pooling: reduce spatial dimensions to (1,1)
...
def plot_scatter_predictions(Y_true_all, Y_pred_all):
    """
    Create a scatter plot for 5 test images per unit (all cells) showing predicted vs real values.
    Y_true_all and Y_pred_all are lists of arrays (each array shape: (N_test, 1)) from each unit.
    For each unit, we randomly select 5 test samples.
    """
    true_vals = []
    pred_vals = []
    for true, pred in zip(Y_true_all, Y_pred_all):
        ...
        if n >= 5:
            ...
            idx = np.arange(n)
        true_vals.extend(true[idx, 0])
        pred_vals.extend(pred[idx, 0])
